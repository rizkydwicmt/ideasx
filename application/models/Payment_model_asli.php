...
class Payment_model extends CI_Model
{
    ...
    public function savePayment($idtrans)
    {
        ...
        $qrM = "INSERT INTO accfin.payment (
         ...
         no_cek_bg_tt)
         VALUES (
          $id, '" . $no_trans . "', to_timestamp('" . $datmaster['dt_payment'] . "','dd/mm/yyyy'), to_timestamp('" . $datmaster['dt_payment'] . "','dd/mm/yyyy'), 
          ...
          now(), '0', '0', '0', '" . $idtrans . "', 
          '" . $datmaster['id_rek_gl'] . "', '" . $datmaster['id_curr'] . "', " . $datmaster['kurs'] . ", '0',
    ...
    public function editPayment($id)
    {
        ...
        foreach ($x->result_array() as $row) {
            $keys = array_keys(array_column($datdetail, 'uid'), $row['idd']);
            if (empty($keys)) {
    ...
    public function getCcproData()
    {
        ...
        $getFilteredValCcpro = !is_null($this->input->post('searching_ccpro')) ? $this->input->post('searching_ccpro') : '';

        $qrM = "SELECT a.id_cc_project, a.cc_project_name
            FROM master.mcc_project a
            WHERE a.isactive='1'
            AND  concat(upper(a.id_cc_project),'',upper(a.cc_project_name)) like upper('%$getFilteredValCcpro%') ";
