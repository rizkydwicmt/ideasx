...
<div id="content" class="content">
    ...
    <div id="container" class="easyui-panel" style="width:100%;height:460px;padding:10px;">
        <table id="dgMain" class="easyui-datagrid" style="width:100%;height:420px;" url="<?= base_url(); ?>sales/Quotsheet/getQuot" singleSelect="true" striped="true" nowrap="false" fitColumns="true" rownumbers="true" pagination="true" toolbar="#toolbarFilter">
            <thead>
                <tr>
                    <th field="id_qs" width="70">Index</th>
                    <th field="no_qs" width="130">Quotsheet #</th>
                    <th field="revision" width="50">Rev #</th>
                    <th field="dt_qs" width="100">Date</th>
                    <th field="nama_rekanan" width="220" align=left>Customer Name</th>
                    <th field="cpdescriptions" width="100">Category</th>
...
<script type="text/javascript">
    ...
    function newData() {
        ...
        $.ajax({
            url: "<?= base_url(); ?>sales/Quotsheet/newQuot",
            method: "POST",
            dataType: 'json',
            data: {
                dtx: tg1 + '_' + tg2
            ...
            success: function(data) {
                window.open('<?= base_url(); ?>sales/Quotsheet/loadQuot', '_self');
            }
        });
    ...
    function editData() {
        ...
        if (row) {
            ...
            $.ajax({
                url: "<?= base_url(); ?>sales/Quotsheet/editQuot",
                method: "POST",
                dataType: 'json',
                data: {
                    dtx: tg1 + '_' + tg2,
    ...
    function revisionData() {
        ...
        if (row) {
            ...
            if (result) {
                ...
                $.ajax({
                    url: "<?= base_url(); ?>sales/Quotsheet/revQuot",
                    method: "POST",
                    dataType: 'json',
                    data: {
                        dtx: tg1 + '_' + tg2,
                ...
                // window.open('<?//= base_url(); ?>sales/Quotsheet/revQuot/' + row.id_qs + 'x' + tg1 + '_' + tg2, '_self');
            }
        }
    }
    ...
    function removeData() {
        ...
        if (row) {
            ...
            if (result) {
                ...
                //alert(row.kd_rekanan);
                $.ajax({
                    url: "<?= base_url(); ?>sales/Quotsheet/destroyQuot",
                    method: "POST",
                    dataType: 'json',
                    data: {
                        id: row.id_qs
    ...
    $(function() {
        $('#dgMain').datagrid({
            ...
            onExpandRow: function(index, row) {
                $('#ddv-' + index).datagrid({
                    url: '<?= base_url(); ?>sales/Quotsheet/getQuotDetail/' + row.id_qs,
                    fitColumns: true,
                    singleSelect: true,
                    nowrap: false,
                    height: 'auto',
    ...
    function printData() {
        ...
        var row = $('#dgMain').datagrid('getSelected');
        if (row) {
            window.open('<?= base_url(); ?>sales/Quotsheet/cetakQuot/' + row.id_qs);
