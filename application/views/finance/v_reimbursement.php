...
<div id="content" class="content">
    ...
    <div id="container" class="easyui-panel" style="width:100%;height:500px;padding:10px;">
        <table id="dgMain" class="easyui-datagrid" style="width:100%;height:460px;;" url="<?= base_url(); ?>accfin/Reimbursement/getReimbursement/<?= $notrans; ?>" singleSelect="true" fitColumns="true" nowrap="false" rownumbers="true" pagination="true" toolbar="#toolbarFilter">
            <thead>
                <tr>
                    <th field="id_settlement" width="70">Index</th>
                    <th field="no_settlement" width="120">Reimbursement #</th>
...
<script type="text/javascript">
    ...
    function newData() {
        ...
        $.ajax({
            url: "<?= base_url(); ?>accfin/Reimbursement/newReimbursement",
            method: "POST",
            dataType: 'json',
            data: {
                dtx: tg1 + '_' + tg2
            ...
            success: function(data) {
                window.open('<?= base_url(); ?>accfin/Reimbursement/loadReimbursement', '_self');
            }
        });
    }
    ...
    function editData() {
        ...
        $.ajax({
            url: "<?= base_url(); ?>accfin/Reimbursement/editReimbursement",
            method: "POST",
            dataType: 'json',
            data: {
                dtx: tg1 + '_' + tg2,
    ...
    function removeData() {
        ...
        if (row) {
            ...
            if (result) {
                ...
                //alert(row.kd_rekanan);
                $.ajax({
                    url: "<?= base_url(); ?>accfin/Reimbursement/destroyReimbursement",
                    method: "POST",
                    dataType: 'json',
                    data: {
                        id: row.id_settlement
    ...
    $(function() {
        $('#dgMain').datagrid({
            ...
            onExpandRow: function(index, row) {
                $('#ddv-' + index).datagrid({
                    url: '<?= base_url(); ?>accfin/Reimbursement/getDetailReimbursement/' + row.id_settlement,
                    // fitColumns: true,
                    singleSelect: true,
                    height: 'auto',
                    //pagination: true,
